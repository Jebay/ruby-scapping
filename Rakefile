...
vegetables_list = ['AIL', 'ARTICHAUT', 'ASPERGE', 'AUBERGINE', 'AVOCAT',
                   ...
                   'HERBE', 'HUILE', 'LAITUE', 'MÂCHE', 'NAVET', 'OIGNON', 'PATATE', 'POIREAU', 'POMME DE TERRE', 'POTIRON', 'RADIS',
                   'RHUBARBE', 'ROQUETTE', 'SALADE', 'SALSIFIS', 'TOMATE',
...
task :web_scraper do
  ...
  page.css('table tr').each do |row|
    ...
    if vegetables_list.any? { |vegetable| name.include?(vegetable) }
      ...
      name_short += '%20FRUIT'
      if !name.include?('POMME DE TERRE')
